{ (c) 29.06.1994 by Mr.Old from Down Raisin, ver. 4.23, 09.02.1995 last upgrade  }
...
label znovu;
begin
if paramcount=1 then if paramstr(1)='386' then pauza486:=0 else if paramstr(1)='486' then pauza486:=15;
...
nacticpszpaku('RMENU.CPS',0,0);zobrazinventar;
...
case cismis of
  1:begin
	if cuchejkod(1{mistnost},1{osoba},{kod:}2,0,0,0) then dodelejdialog;
...
{if pouzij(1,0,0) then begin
	pistext(manx,many-20,'Pr vˆ ‘eru tohle jabko,',23);
	pistext(manx,many-20,'asi mi bude zle...',18);
	zrusupl:=1;
end;
...
if c=#13 then begin p[0].vych.kam:=cismis;mistnost(0);end;
...
if (r.bx=2) then begin
	inc(ikon);if ikon=pocikon-2 then begin
		ikon:=1;use[1]:=mam[0];use[2]:=0;use[3]:=0;
	end;
end;
